...
def preprocess_feature_array(s, n_mels=128, drop_mels=40):
    """
    s: np.ndarray from .npz (2D: [T, n_mels] or [n_mels, T])
    Returns:
...
def fit_streaming_pca_kmeans(data_path: Path, n_components: int = 256, k: int = 8, batch_files: int = 512
    , sample_size: int = 8000, random_state: int = 104):
...
def assign_labels_and_save(data_path: Path, scaler: StandardScaler, ipca: IncrementalPCA, kmeans: MiniBatchKMeans
    ...
    , topn: int = 10, save_downsampled_dir: Path | None = None):
    """
    Streaming pass that:
    ...
    # We iterate in the same order as used_paths to keep indexes aligned.
    i = 0
    files_iter = (p for p in used_paths)
    while True:
        batch = []
        bpaths = []
        try:
            for _ in range(batch_files):
                ...
                with np.load(p, mmap_mode="r") as z:
                    if key not in z.files: # Possible mapping bug if any files are skipped. Should not happen.
                        continue
                    s = np.squeeze(z[key])
        ...
        d2_assigned = d2_all[np.arange(len(labs)), labs]

        # Record outputs into full arrays.
        for j, pth, d2 in zip(labs, bpaths, d2_assigned):
            labels[i] = j
            dist2_assigned[i] = d2
            # maintain top-N heap for cluster j
            h = nearest_heaps[j]
