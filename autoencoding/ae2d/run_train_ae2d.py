...
def main():
    ...
    ap.add_argument("--model_dir", type=str, default=None, help="Directory to load model from.")
    ...
    with np.load(model_dir / "model_results.npz", mmap_mode="r") as d:
        train_files = [Path(p) for p in d["train_files"]]
        val_files = [Path(p) for p in d["val_files"]]
        test_files = [Path(p) for p in d["test_files"]]

    with np.load(train_files[0], mmap_mode="r") as z:
    ...
    n_files = len(train_files) + len(val_files) + len(test_files)
    n_train = len(train_files)
    n_val = len(val_files)
    n_test = len(test_files)
    print(f"Total files = {n_files}, train = {n_train}, val = {n_val}, test = {n_test}")
