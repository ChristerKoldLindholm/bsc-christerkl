...
def stream_segments(files, batch_segments:int, segment_sec:float=3.0
                    , clip_sec:float=270.0,key:str="feature"):
    """
...
def fit_kmeans_streaming(files, transform_fn, n_pc:int, k:int, batch_segments:int
                         , segment_sec:float, clip_sec:float, random_state:int=104):
    """
...
def main():
    ...
    scaler_scale[scaler_scale == 0.0] = 1.0  # 0-dividion safeguard.
